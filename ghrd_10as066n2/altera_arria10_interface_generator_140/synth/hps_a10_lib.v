...
generate
if (DWIDTH != 16) begin
    alentar#(.DATA_WIDTH(DWIDTH)) wdata_alen (s_w_data,  d_w_data);
    alentar#(.DATA_WIDTH(32))  awaddr_alen   (s_aw_addr, d_aw_addr);
    alentar#(.DATA_WIDTH(32))  araddr_alen   (s_ar_addr, d_ar_addr);
    alentar#(.DATA_WIDTH( 4))  ar_cache_alen (s_ar_cache,d_ar_cache);
    alentar#(.DATA_WIDTH( 4))  ar_id_alen    (s_ar_id,   d_ar_id);
    alentar#(.DATA_WIDTH( 4))  ar_len_alen   (s_ar_len,  d_ar_len);
    alentar#(.DATA_WIDTH( 5))  ar_ar_user    (s_ar_user, d_ar_user);
    alentar#(.DATA_WIDTH( 4))  aw_cache_alen (s_aw_cache,d_aw_cache);
    alentar#(.DATA_WIDTH( 4))  aw_id_alen    (s_aw_id,   d_aw_id);
    alentar#(.DATA_WIDTH( 4))  aw_len_alen   (s_aw_len,  d_aw_len);
    alentar#(.DATA_WIDTH( 5))  aw_ar_user    (s_aw_user, d_aw_user);
    alentar#(.DATA_WIDTH( 4))  w_id_alen     (s_w_id,    d_w_id);
...
generate
if (DWIDTH != 16) begin
    alentar#(.DATA_WIDTH(DWIDTH)) rdata_alen  (s_r_data,  d_r_data);
    alentar#(.DATA_WIDTH(4)) b_id_alen  (s_b_id,  d_b_id);
    alentar#(.DATA_WIDTH(4)) r_id_alen  (s_r_id,  d_r_id);
    parameter WIDTH = DWIDTH;
end
else begin
